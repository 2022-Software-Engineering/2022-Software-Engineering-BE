...
def requestPlantDetails(plantID):
    ...
    if resultCode == '91':
        ...
        r['plantDescription'] = targetStrRemover(item['clCodeDc']) #과 설명
        r['countryOfOrigin'] = targetStrRemover(item['orgplce']) #원산지
        r['flowerExist'] = targetStrRemover(item['flwrInfo']) #꽃
        r['characteristic'] = targetStrRemover(item['chartrInfo']) #특성
        r['rootForm'] = targetStrRemover(item['rdxStleNm']) #뿌리형태
        r['growthPattern'] = targetStrRemover(item['grwtInfo']) #생장형
        r['growthRate'] = targetStrRemover(item['grwtseVeNm']) #생장속도
        r['lightAmount'] = targetStrRemover(item['lighttInfo']) #광
        r['watering'] = targetStrRemover(item['waterCycleInfo']) #물주기
        r['demangeOfBugs'] = targetStrRemover(item['dlthtsInfo']) #병충해
        r['manageLevel'] = targetStrRemover(item['manageLevelNm']) #관리수준
        r['manageDemand'] = targetStrRemover(item['manageDemandNm']) #관리요구도
        r['fertilizer'] = targetStrRemover(item['frtlzrInfo']) #비료
